...
import com.medvision.vrmed.bluetooth.factory.CommandHolder;
import com.medvision.vrmed.bluetooth.factory.ParseData;
import com.medvision.vrmed.bluetooth.factory.ResultData;
import com.medvision.vrmed.utils.ToHexByteUtils;
        ...
        if (mService == null) {
            ...
            mBLEStatusChangeReceiver.setOnBLEStatusChangeListener(new BLEStatusChangeReceiver.OnBLEStatusChangeListener() {
                ...
                public void onDataChange(String uuid, byte[] value, String type) {
                    ...
                    Log.e("+-----:onDataChange", s);
                    ResultData parse = ParseData.parse(value);
                    if (parse!= null) {
                        if (parse.getSu()) {
                            switch (parse.getType()) {
                                case ParseData.NOTHING:
                                    break;
                                case ParseData.HANDLER:
                                    mHandler.sendEmptyMessage(5);
                                    break;
                                case ParseData.NEXT:
                                    mService.writeRXCharacteristic(parse.getCommandHolder());
                                    break;
                            }
                        } else {
...
////                        mService.writeRXCharacteristic(ToHexByteUtils.hexStringToByte("8100810100"));
//                        mService.writeRXCharacteristic(BTFactory.createConnectCMD(null));
...
//                    }else if(s.startsWith("03000110")){
//                        mService.writeRXCharacteristic(ToHexByteUtils.hexStringToByte("0300021001010F01010101010101010101010101"));
//                    }else if(s.startsWith("03000210")){
//                        mService.writeRXCharacteristic(ToHexByteUtils.hexStringToByte("0300830400000100"));
//                    }
                }
...
//                finish();
    ...
    private void connectBLEDevice() {
        if (mService != null) {
            Boolean s = mService.connect(mDeviceAddress);
        }
    }
    ...
    public void writeRXCharacteristic(CommandHolder commandHolder) {
        if (mService != null)
            mService.writeRXCharacteristic(commandHolder);
    }
}
